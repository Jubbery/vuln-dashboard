...
store.subscribe(() => {
  const { pageSize, sort, gridDensity, columns, themeMode, overview } = store.getState().ui;
  if (pageSize !== lastPrefs.pageSize || sort !== lastPrefs.sort ||
      gridDensity !== lastPrefs.gridDensity || columns !== lastPrefs.columns ||
      themeMode !== lastPrefs.themeMode || overview !== lastPrefs.overview) {
    lastPrefs = { pageSize, sort, gridDensity, columns, themeMode, overview };
    savePreferences({ pageSize, sort, gridDensity, columns, themeMode, overview });
  }
});
